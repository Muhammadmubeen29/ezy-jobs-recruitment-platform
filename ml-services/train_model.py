...
def generate_training_data(num_examples=200):
    ...
    try:
        from data_generation.synthetic_data_generator import (
            EZYJobsSyntheticDataGenerator,
        )

        print(f"{Fore.CYAN}🎯 Initializing data generator...{Style.RESET_ALL}")
        generator = EZYJobsSyntheticDataGenerator()
...
def print_final_results(success, training_data=None):
    ...
    if success:
        print(
            f"\n{Fore.GREEN + Style.BRIGHT}🎉 SUCCESS: Your EZY Jobs AI model is trained and ready!{Style.RESET_ALL}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train EZY Jobs AI Model")
    parser.add_argument(
        "--data-only",
        action="store_true",
