...
const createInterviewerRating = asyncHandler(async (req, res) => {
  ...
  const isEmailSent = await sendEmail(res, {
    ...
    subject: 'EZY Jobs - New Rating Received',
...
const getAllInterviewerRatings = asyncHandler(async (req, res) => {
  const {
    rating: ratingQuery,
    interviewerId: interviewerQuery,
    recruiterId: recruiterQuery,
    jobId: jobQuery,
  ...
  const filter = {};
  if (ratingQuery) filter.rating = Number(ratingQuery);
  if (interviewerQuery) filter.interviewerId = interviewerQuery;
  if (recruiterQuery) filter.recruiterId = recruiterQuery;
  if (jobQuery) filter.jobId = jobQuery;
  if (contractQuery) filter.contractId = contractQuery;
  if (search) filter.feedback = { $regex: search, $options: 'i' };

  const interviewerRatings = await InterviewerRating.find(filter)
  ...
  if (!interviewerRatings || interviewerRatings.length === 0) {
    return res.status(StatusCodes.OK).json({
      success: true,
      message: 'No interviewer ratings found. Please adjust your filters and try again.',
      count: 0,
      interviewerRatings: [],
      timestamp: new Date().toISOString(),
    });
...
const getInterviewerRatingById = asyncHandler(async (req, res) => {
  const interviewerRating = await InterviewerRating.findById(req.params.id)
    .populate('interviewerId', 'firstName lastName email')
    .populate('recruiterId', 'firstName lastName email')
    .populate('jobId', 'title company')
    .populate('contractId', '_id');
...
const updateInterviewerRating = asyncHandler(async (req, res) => {
  ...
  if (rating !== undefined) interviewerRating.rating = rating;
  if (feedback) interviewerRating.feedback = feedback;

  const updatedInterviewerRating = await interviewerRating.save();
...
const deleteInterviewerRating = asyncHandler(async (req, res) => {
  ...
  const deletedRating = await InterviewerRating.findByIdAndDelete(req.params.id);
...
const getInterviewerRatingsByJob = asyncHandler(async (req, res) => {
  ...
  const interviewerRatings = await InterviewerRating.find({ jobId })
    .populate('interviewerId', 'firstName lastName email')
    .populate('recruiterId', 'firstName lastName email')
    .populate('jobId', 'title company')
    .populate('contractId', '_id');
  if (!interviewerRatings || interviewerRatings.length === 0) {
    return res.status(StatusCodes.OK).json({
      success: true,
      message: 'No interviewer ratings found for this job posting. Please check and try again.',
      count: 0,
      interviewerRatings: [],
      timestamp: new Date().toISOString(),
    });
...
const getInterviewerRatingsByContract = asyncHandler(async (req, res) => {
  ...
  const interviewerRatings = await InterviewerRating.find({ contractId })
    .populate('interviewerId', 'firstName lastName email')
    .populate('recruiterId', 'firstName lastName email')
    .populate('jobId', 'title company')
    .populate('contractId', '_id');
  if (!interviewerRatings || interviewerRatings.length === 0) {
    return res.status(StatusCodes.OK).json({
      success: true,
      message: 'No interviewer ratings found for this contract. Please check and try again.',
      count: 0,
      interviewerRatings: [],
      timestamp: new Date().toISOString(),
    });
