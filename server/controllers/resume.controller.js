...
const createResume = asyncHandler(async (req, res) => {
  ...
  const existingProfile = await Resume.findOne({ userId });
...
const getAllResumes = asyncHandler(async (req, res) => {
  ...
  const profiles = await Resume.find(query)
...
const updateResume = asyncHandler(async (req, res) => {
  ...
  const updatedProfile = await Resume.findOneAndUpdate(
    { userId },
    validatedData,
    { new: true }
  );
...
const deleteResume = asyncHandler(async (req, res) => {
  ...
  const deletedProfile = await Resume.findOneAndDelete({ userId });
...
const updateResumeById = asyncHandler(async (req, res) => {
  ...
  const updatedProfile = await Resume.findByIdAndUpdate(
    profileId,
    validatedData,
    { new: true }
  ).populate('userId', 'firstName lastName email phone');
  ...
  const isEmailSent = await sendEmail(res, {
    ...
    to: updatedProfile.userId.email,
    subject: 'EZY Jobs - Resume Updated',
    html: generateEmailTemplate({
      firstName: updatedProfile.userId.firstName,
...
const deleteResumeById = asyncHandler(async (req, res) => {
  ...
  const deletedProfile = await Resume.findByIdAndDelete(profileId);
  ...
  const isEmailSent = await sendEmail(res, {
    ...
    subject: 'EZY Jobs - Resume Deleted',
    html: generateEmailTemplate({
